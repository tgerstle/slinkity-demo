...
/**
 * Note these gql clients use GET
 * The client side browser client might be updated
 * to use POST
 * This is to accomodate the 11ty cache fetching
 * @param {*} settings 
...
const getClient = (settings = {
    ...
    magentoUrl: false,
    wordpressUrl: false,
    htauthUsername: false,
    htauthPassword: false,
    browser: false,
    store: 'default'
}) => {

    let baseUrl = ''
    if (!!settings.source && settings.source == 'magento' && !!settings.magentoUrl) {
        baseUrl = settings.magentoUrl;
    }
    if (!!settings.source && settings.source == 'wordpress' && !!settings.wordpressUrl) {
        baseUrl = settings.wordpressUrl;
    }
    let headers = {}
    if (
        typeof settings.htauthPassword != 'undefined' &&
        settings.htauthPassword &&
        settings.htauthPassword.length > 0 &&
        typeof settings.htauthUsername != 'undefined' &&
        settings.htauthUsername &&
        settings.htauthUsername.length > 0
    ) {
    ...
    //serverside
    if (typeof settings.browser == 'undefined' || !settings.browser) {
        if (!!settings.source && settings.source == 'wordpress' && !!settings.wordpressUrl) {
            client = {
                browser: false,
                ...
                query: async (query, vars) => {
                    ...
                    //for GET use
                    //may have to remove json type from heders
                    // let url = baseUrl + '?' + q;
                    ...
                    let url = baseUrl + '#' + hash;//+ '?' + q;
                    console.log('wordpress query ' + hash);
                    let res = await EleventyFetch(url, {
                        duration: settings.duration,
                        type: "json",
                        verbose: false,
        ...
        } else if (!!settings.source && settings.source == 'magento' && !!settings.magentoUrl) {
            client = {
                browser: false,
                source: settings.source,
                query: async (query, vars) => {
...
* @returns 
*/
const graphqlQuery = async function (query, vars = {}, source = 'magento', config = { siteId: 'dn', magentoUrl: '', keepDataLevel: false }) {
